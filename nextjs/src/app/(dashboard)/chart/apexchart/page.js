...
// 1. Chart configurations from public/assets/js/apexchart.js
...
export default function Page() {
  useEffect(() => {
    // 2. Initialize ApexCharts in useEffect
    if (typeof window !== "undefined" && window.ApexCharts) {
      let chartEmployee;
      let chartAttendanceRate;
      let chartPayrollSummary;
      let averageTeamKPI;
      let chartDailySalesStates;
      let chartStocksPrices;

      const chartEmployeeEl = document.querySelector("#chartEmployee");
      if (chartEmployeeEl) {
        chartEmployee = new window.ApexCharts(chartEmployeeEl, chartEmployeeConfig);
        chartEmployee.render();
      }

      const chartAttendanceRateEl = document.querySelector("#chartAttendanceRate");
      if (chartAttendanceRateEl) {
        chartAttendanceRate = new window.ApexCharts(chartAttendanceRateEl, chartAttendanceRateConfig);
        chartAttendanceRate.render();
      }

      const chartPayrollSummaryEl = document.querySelector("#chartPayrollSummary");
      if (chartPayrollSummaryEl) {
        chartPayrollSummary = new window.ApexCharts(chartPayrollSummaryEl, chartPayrollSummaryConfig);
        chartPayrollSummary.render();
      }

      const averageTeamKPIEl = document.querySelector("#averageTeamKPI");
      if (averageTeamKPIEl) {
        averageTeamKPI = new window.ApexCharts(averageTeamKPIEl, averageTeamKPIConfig);
        averageTeamKPI.render();
      }

      const chartDailySalesStatesEl = document.querySelector("#chartDailySalesStates");
      if (chartDailySalesStatesEl) {
        chartDailySalesStates = new window.ApexCharts(chartDailySalesStatesEl, chartDailySalesStatesConfig);
        chartDailySalesStates.render();
      }

      const chartStocksPricesEl = document.querySelector("#chartStocksPrices");
      if (chartStocksPricesEl) {
        chartStocksPrices = new window.ApexCharts(chartStocksPricesEl, chartStocksPricesConfig);
        chartStocksPrices.render();
      }

      // Cleanup to destroy chart instances and free memory on component unmount
      return () => {
        if (chartEmployee) chartEmployee.destroy();
        if (chartAttendanceRate) chartAttendanceRate.destroy();
        if (chartPayrollSummary) chartPayrollSummary.destroy();
        if (averageTeamKPI) averageTeamKPI.destroy();
        if (chartDailySalesStates) chartDailySalesStates.destroy();
        if (chartStocksPrices) chartStocksPrices.destroy();
      };
    }
  }, []);
